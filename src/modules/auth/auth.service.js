...
export const login = asyncHandler(async (req, res, next) => {
  ...
  return res
    ...
    .json({ 
      ...
      access_token: generateToken({ payload: { id: user._id, email: user.email } },{expiresIn:process.env.ACCESS_TOKEN_EXPIRE}),
      refresh_token: generateToken({ payload: {  email: user.email,id: user._id } },{expiresIn:process.env.REFRESH_TOKEN_EXPIRE})
      });
});
...
export const newAccessToken = asyncHandler(async(req,res,next) =>{
  ...
  const access_token = generateToken({ payload},{expiresIn:process.env.ACCESS_TOKEN_EXPIRE});
  return res.status(200).json({success:true,result:{access_token:access_token}})
...
export const verifyOTP = asyncHandler(async(req,res,next) =>{
  ...
  return res.status(200).json({success:true,message:"OTP Verified Successfully , Try To login.."})
...
export const forgetPassword = asyncHandler(async(req,res,next) =>{
  ...
  return res.status(200).json({success:true,message:"OTP Send Succsessfully.."})
});
